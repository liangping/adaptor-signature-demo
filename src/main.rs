...
fn main() {
    ...
    // Bob randomly picks a secret: `t` as hash lock and create CETs.
    ...
    let repayment_secret = Scalar::random(&mut rand::thread_rng()); // generated on the day of repayment
    ...
    // if Bob dese not decrypt, nothing will happen.
    let valid_signature = adaptor_signature.adapt(repayment_secret).unwrap(); // secret generated on the day of loan repayment
    ...
    // The decrypted signature and the adaptor signature allow an
    // observer to deduce the adaptor secret.
    let revealed_payment_secret: MaybeScalar = adaptor_signature
        .reveal_secret(&valid_signature)
        .expect("should compute adaptor secret from decrypted signature");

    // This is the crucial step: the `revealed_payment_secret` from the signed adaptor
    // signature is the same as the `repayment_secret` generated by Bob when he repaid
    // the loan.
    assert_eq!(
        revealed_payment_secret,
        MaybeScalar::Valid(repayment_secret)
    );
    println!(
        "{:?}\n {:?}",
        revealed_payment_secret.serialize(),
